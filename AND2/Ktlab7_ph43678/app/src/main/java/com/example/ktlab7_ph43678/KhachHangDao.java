...
public class KhachHangDao {
    ...
    public boolean update(KhachHang khachHang) {
        ...
        values.put("NGAYSINH", khachHang.getNgaysinh());
        // Nếu thành công sẽ trả về giá trị tương tự số hàng mà dữ liệu được add trong bảng
        long row = database.update("KHACHHANG", values,"MAKH=?",new String[]{String.valueOf(khachHang.getMakh())});
        return (row > 0);
    }

    public boolean delete(int id) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        long row = db.delete("KHACHHANG", "MAKH=?", new String[]{String.valueOf(id)});
        return (row > 0);
    }
}
